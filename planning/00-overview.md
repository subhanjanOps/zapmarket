...
2b. **[Stage 2b — Product Image Upload via MinIO](02b-image-upload-minio.md)** 🔜 **Next (inserted, top priority)** — not in the original checklist; bumped ahead of Stage 3 at explicit request. Real multipart upload for product images, backed by the MinIO bucket already provisioned in `docker-compose.yml`.
